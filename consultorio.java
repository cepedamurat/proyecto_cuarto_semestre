...
public class consultorio extends JFrame{
    ...
    private void creaBarraDeMenu() {
        ...
        menuAcercaDe.addActionListener((ActionEvent e) -> {
            JOptionPane.showMessageDialog(this, "Sistema de administración de consultorios\nVersión 1.0\nDesarrollado por:\nCepeda Beltrán César Murat\nMartínez Muñiz Alan Jael\nRufino Moreno Susana Abigail\nTorres Abonce Roberto Alejandro");
        });
    ...
    private void llenaInventario(JPanel panelInventario, String filtro) {
        panelInventario.removeAll();

        // Definir los nombres de las columnas
        String[] columnNames = {"ID", "Nombre", "Descripción", "Precio", "Cantidad", "Acciones"};

        // Obtener los inventarios de la base de datos
        ArrayList<Inventario> inventarios = bd.buscaInventarioPorNombre(filtro);

        // Crear los datos para la tabla
        Object[][] data = new Object[inventarios.size()][6];
        for (int i = 0; i < inventarios.size(); i++) {
            Inventario inv = inventarios.get(i);
            data[i][0] = inv.getId();
            data[i][1] = inv.getNombre();
            data[i][2] = inv.getDescripcion();
            data[i][3] = String.format("%.2f", inv.getPrecio());
            data[i][4] = inv.getCantidad();
            data[i][5] = ""; // Placeholder para los botones
        }
        ...
        javax.swing.table.DefaultTableModel model = new javax.swing.table.DefaultTableModel(data, columnNames) {
            ...
            // Solo la columna de acciones es editable (para los botones)
            return column == 5;
            }
        };
        ...
        table.getColumnModel().getColumn(4).setPreferredWidth(60);   // Cantidad
        table.getColumnModel().getColumn(5).setPreferredWidth(130);  // Acciones

        // Renderizador para los botones en la columna de acciones
        table.getColumn("Acciones").setCellRenderer(new javax.swing.table.TableCellRenderer() {
            @Override
        ...
        table.getColumn("Acciones").setCellEditor(new javax.swing.DefaultCellEditor(new JTextField()) {
            JPanel panel = new JPanel(new FlowLayout(FlowLayout.CENTER, 5, 0));
            JButton btnEditar = new JButton("Editar");
            JButton btnEliminar = new JButton("Eliminar");
            int editingRow = -1;
            ...
            btnEditar.addActionListener(e -> {
                if (editingRow >= 0) {
                    String nuevaCantidadStr = JOptionPane.showInputDialog(panelInventario, "Ingrese la nueva cantidad:", table.getValueAt(editingRow, 4));
                    if (nuevaCantidadStr != null) {
                        try {
                            int nuevaCantidad = Integer.parseInt(nuevaCantidadStr.trim());
                            if (nuevaCantidad >= 0) {
                                int id = (int) table.getValueAt(editingRow, 0);
                                bd.actualizarCantidadInventario(id, nuevaCantidad);
                                llenaInventario(panelInventario, filtro); // Actualizar el inventario después de editar
                            } else {
                                JOptionPane.showMessageDialog(panelInventario, "La cantidad debe ser mayor o igual a cero.", "Error", JOptionPane.ERROR_MESSAGE);
                            }
                        } catch (NumberFormatException ex) {
                            JOptionPane.showMessageDialog(panelInventario, "Ingrese un número válido.", "Error", JOptionPane.ERROR_MESSAGE);
                        }
                    }
                }
                fireEditingStopped();
            ...
            btnEliminar.addActionListener(e -> {
                if (editingRow >= 0) {
                int respuesta = JOptionPane.showConfirmDialog(panelInventario, "¿Estás seguro de que deseas eliminar este objeto del inventario?", "Confirmar eliminación", JOptionPane.YES_NO_OPTION);
                if (respuesta == JOptionPane.YES_OPTION) {
                    int id = (int) table.getValueAt(editingRow, 0);
                    bd.borrarInventario(id);
                    llenaInventario(panelInventario, filtro); // Actualizar el inventario después de eliminar
                }
                }
                fireEditingStopped();
            });
    ...
    private Component creaVentanaInventario() {
        JPanel panel = new JPanel(new BorderLayout());
        panel.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));

        JPanel panelSuperior = new JPanel(new BorderLayout());
        JTextField textoBusqueda = new JTextField();
        JButton btnBuscar = new JButton("Buscar");
        JButton btnAgregar = new JButton("Agregar");

        JPanel panelBusqueda = new JPanel(new BorderLayout());
        panelBusqueda.add(textoBusqueda, BorderLayout.CENTER);
        ...
        // Panel para mostrar los inventarios
        ...
        llenaInventario(panelInventario, textoBusqueda.getText());
        ...
        btnAgregar.addActionListener(e -> {
            VentanaAgregarObjeto ventanaAgregar = new VentanaAgregarObjeto();
            ventanaAgregar.setLocationRelativeTo(this);
            ventanaAgregar.setVisible(true);
            // Actualizar inventario después de agregar
            llenaInventario(panelInventario, "");
        });
        return panel;
    }
